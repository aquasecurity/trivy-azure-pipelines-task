...
sed  "s/VERSION_PLACEHOLDER/${SIMPLE_VERSION}/" vss-extension.dev.json.tmpl > vss-extension.dev.json
SPLIT_VERSION=$(echo $SIMPLE_VERSION | awk -F. {'printf "\"Major\":%d,\"Minor\":%d,\"Patch\":%d", $1, $2, $3'})
echo "Split version: ${SPLIT_VERSION}"
sed "s/VERSION_PLACEHOLDER/${SPLIT_VERSION}/" trivy-task/task.json.tmpl > trivy-task/task.json
make package-dev
