"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var os = require("os");
var util = require("util");
var tool = require("azure-pipelines-tool-lib");
var task = require("azure-pipelines-task-lib/task");
var templateUrl = "https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/junit.tpl";
function run() {
    return __awaiter(this, void 0, void 0, function () {
        var url, localPath, downloadPath, binPath, templatePath, outputPath, runner, result, publisher, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 5, , 6]);
                    if (os.platform() != "linux") {
                        throw new Error("cannot run on non-Linux system");
                    }
                    url = getArtifactURL();
                    localPath = "/tmp/trivy.tar.gz";
                    task.rmRF(localPath);
                    return [4 /*yield*/, tool.downloadTool(url, localPath)];
                case 1:
                    downloadPath = _a.sent();
                    return [4 /*yield*/, tool.extractTar(downloadPath, "/tmp")];
                case 2:
                    _a.sent();
                    binPath = "/tmp/trivy";
                    return [4 /*yield*/, task.exec('chmod', ["+x", binPath])];
                case 3:
                    _a.sent();
                    templatePath = "/tmp/junit.tpl";
                    task.rmRF(templatePath);
                    return [4 /*yield*/, tool.downloadTool(templateUrl, templatePath)];
                case 4:
                    _a.sent();
                    outputPath = "/tmp/trivy.xml";
                    task.rmRF(outputPath);
                    runner = task.tool(binPath);
                    runner.line(util.format("fs --exit-code 13 --output %s --template \"@%s\" --format template --security-checks vuln,config,secret", outputPath, templatePath));
                    runner.arg(task.cwd());
                    result = runner.execSync();
                    if (result.code === 0) {
                        task.setResult(task.TaskResult.Succeeded, "No problems found.");
                    }
                    else if (result.code === 13) {
                        task.setResult(task.TaskResult.Failed, "Failed (problems found).");
                    }
                    else {
                        task.setResult(task.TaskResult.Failed, "Error: " + result.stderr);
                    }
                    publisher = new task.TestPublisher('JUnit');
                    publisher.publish(outputPath, 'true', '', '', "trivy", 'true', "trivy");
                    return [3 /*break*/, 6];
                case 5:
                    err_1 = _a.sent();
                    task.setResult(task.TaskResult.Failed, err_1.message);
                    return [3 /*break*/, 6];
                case 6: return [2 /*return*/];
            }
        });
    });
}
function getArtifactURL() {
    var version = task.getInput('version', true);
    if (version === undefined) {
        return "";
    }
    var cleanVersion = version;
    if (version.length > 0 && version[0] == 'v') {
        cleanVersion = version.substring(1);
    }
    else {
        version = "v" + version;
    }
    var arch = "";
    switch (os.arch()) {
        case "arm":
            arch = "ARM";
            break;
        case "arm64":
            arch = "ARM64";
            break;
        case "s390x":
            arch = "s390x";
            break;
        case "x64":
            arch = "64bit";
            break;
        case "ia32":
            arch = "32bit";
            break;
        default:
            throw new Error("unsupported architecture: " + os.arch());
    }
    var artifact = util.format("trivy_%s_Linux-%s.tar.gz", cleanVersion, arch);
    return util.format("https://github.com/aquasecurity/trivy/releases/download/%s/%s", version, artifact);
}
run();
